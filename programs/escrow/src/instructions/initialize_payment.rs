...
pub struct InitializePayment<'info> {
    ...
    #[account(
        init,
        payer = payer,
        space = 8 + 2 * 32 + condition_content.len(), 
        seeds = [
    ...
    ///CHECK: There is no data to check in the lockup account
    #[account(
        seeds = [
    ...
    transfer(
        CpiContext::new(
            ...
            Transfer {
                    ...
                    to: lockup.to_account_info(), //lub .clone() jeśli lockup jest AccountInfo
                }
        ),
        amount
    )?;
